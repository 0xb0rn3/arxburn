//! SHA-256, implemented here rather than pulled from a crate.
//!
...
//! burn tool on. It is FIPS 180-4, checked against the standard vectors in the tests below.
...
impl Sha256 {
    ...
    pub fn update(&mut self, mut data: &[u8]) {
        ...
        // finish any partial block first, then run whole blocks straight off the input
        if self.buflen > 0 {
...
mod tests {
    ...
    // hashes in 4MiB reads, so a bug that only shows on odd splits would be a wrong verdict.
    #[test]
